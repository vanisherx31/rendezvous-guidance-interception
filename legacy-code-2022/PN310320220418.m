...
O = [0 0]
vT = [-150 -500]; %defining the target velocity
vTmag = sqrt((vT(1,1))^2 + (vT(1,2))^2); ; %target velocity magnitude
rT = [9000000 100000]; %defining the target position
px = rT(1,1);
py = rT(1,2);
vx = vT(1,1);
vy = vT(1,2);
...
vImag = vTmag + 9000.000; %defining velocity magnitude for interceptor
...
%      
     vIPN(i,1) = vImag*cosd(mbn(i,3));
     vIPN(i,2) = vImag*sind(mbn(i,3));
...
%     hold on;
    ...
    hold on;
    plot([rIPN(i,1),pT(i,1)],[rIPN(i,2),pT(i,2)],'LineWidth',2);
    hold on;
