...
O = [0 0];
Xpls1 = [1000000 0]
% vT = [50 -50]; %defining the target velocity
vT = [-50 50]; %defining the target velocity
vTmag = sqrt((vT(1,1))^2 + (vT(1,2))^2); ; %target velocity magnitude
rT = [10000 1000]; %defining the target position
px = rT(1,1);
py = rT(1,2);
vx = vT(1,1);
vy = vT(1,2);
...
%     rTmag(i,1) = norm(rfT); %Target position magnitude from origin
     ...
     theta(i,2) = deg2rad(theta(i,1));
     theta(i,3) = vectorangleR2([rIPN(i,1),rIPN(i,2)],[pT(i,1),pT(i,2)],[pT(i,1),pT(i,2)],[vT(i,1),vT(i,2)]);
     theta(i,4) = deg2rad(theta(i,3));  %angle between target velocity vector and LOS
     
     alpha(i,1) = vectorangle([O(1,1),O(1,2)],[pT(i,1),pT(i,2)],[O(1,1),O(1,2)],[Xpls1(1,1),Xpls1(1,2)]);
     alpha(i,2) = deg2rad(alpha(i,1));
     alpha(i,3) = vectorangleR2([Xpls1(1,1),Xpls1(1,2)],[O(1,1),O(1,2)],[pT(i,1),pT(i,2)],[O(1,1),O(1,2)]);
     alpha(i,4) = deg2rad(alpha(i,3)); %angle between target position vector and Xaxis positive
     lambda(i,1) = vectorangle([rIPN(i,1),rIPN(i,2)],[pT(i,1),pT(i,2)],[O(1,1),O(1,2)],[Xpls1(1,1),Xpls1(1,2)]);
     lambda(i,2) = deg2rad(lambda(i,1));
     lambda(i,3) = vectorangleR2([O(1,1),O(1,2)],[Xpls1(1,1),Xpls1(1,2)],[rIPN(i,1),rIPN(i,2)],[pT(i,1),pT(i,2)]);
     lambda(i,4) = deg2rad(lambda(i,3)); %angle between LOS and Xaxis positive
     
     sintheta(i,1) = sind(theta(i,3));  %sin in degrees
     sintheta(i,2) = sin(theta(i,3));  %sin in rads
     
     vTs(i,1) = pT(i,1);
     vTs(i,2) = pT(i,2) + vTmag*sind(theta(i,3)+alpha(i,3));
     vTc(i,1) = pT(i,1) + vTmag*cosd(theta(i,3)+alpha(i,3));
     vTc(i,2) = pT(i,2);
...
%      (vTmag)*(sind(theta(i,1)))
       lambdadot(i,1) = ((vTmag)/(rTmag(i,1)))*sind(theta(i,3)); %with origin to target as the radius arm
       lambdadot(i,2) = ((vTmag)/(rTmag(i,2)))*sind(theta(i,3)); %with interceptor to target as the radius arm
...
%      
     ...
     vIPNs(i,1) = rIPN(i,1);
     vIPNs(i,2) = rIPN(i,2) + vImag*sind(mbn(i,5)+ alpha(i,3));
     vIPNc(i,1) = rIPN(i,1) + vImag*sind(mbn(i,5)+ alpha(i,3));
     vIPNc(i,2) = rIPN(i,2);
...
%      pr(i,1) = sindelta(i,2)*sintheta(i,1);
%      pr(i,2) = sindelta(i,3)*sintheta(i,1);
%      pr(i,3) = sindelta(i,2)*sintheta(i,2);
%      pr(i,4) = sindelta(i,3)*sintheta(i,2);
%      pr(i,5) = d(i,1);
%      if i == 1
%          pr(i,6) = 0;
...
%     hold on;0
    plot(vT(i,1),vT(i,2),'h', 'MarkerSize' ,10, 'MarkerFaceColor', 'b'); %produces dot on the plot
    hold on;
...
%     hold on;
    ...
    hold on;
    axis([-15000 50000 -10000 30000])
    hold on;
    pause(0.00000005)
    
    angl2(i,1) = lambda(i,3);
    angl2(i,2) = lambda(i,4);
    angl2(i,3) = theta(i,3);
    angl2(i,4) = theta(i,4);
    angl2(i,5) = lambda(i,3)+theta(i,3);
    angl2(i,6) = lambda(i,3)-theta(i,3);
...
%     plot(rIPN(i,1),rIPN(i,2),'s', 'MarkerSize' ,6, 'MarkerFaceColor', 'r'); %produces dot on the plot
%     hold on;
%     angleplot(pT(i,1),pT(i,2),lambda(i,2),(lambda(i,2)-theta(i,2)),vTmag)
%     hold on;
%     rIPN(i,1),rIPN(i,2)
%     angleplot(rIPN(i,1),rIPN(i,2),lambda(i,2),(lambda(i,2)+ mbn(i,4)),vIPNmag)
%     hold on;
